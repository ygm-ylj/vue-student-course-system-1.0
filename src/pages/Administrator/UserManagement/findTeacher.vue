<template>
  <div>
    <el-row>
      ...
      <el-col :span="6">
          <el-input type="number" v-model="searchId"  placeholder="输入教师编号搜索" clearable></el-input>
      </el-col>
      <el-col :span="2">
  ...
  methods: {
    ...
    getList () {
      ...
      let data = this.list.filter((item, index) =>
        item.teacherName.includes(this.searchData)
      )
    ...
    getListId () {
      // es6过滤得到满足搜索条件的展示数据list
      let data = this.list.filter((item, index) =>
        item.teacherId.includes(this.searchId)
      )
      this.data = data.filter((item, index) =>
        index < this.page * this.limit && index >= this.limit * (this.page - 1)
      )
